...
bbbserial () {
   ...
   if [ "$1" == "--old" ]; then
       # OLD METHOD: special case for rs01, only if specified by 
       SERIAL=$(hexdump -e '8/1 "%c"' "${EEPROM}" -s 80 -n 9 2>&1)
       if [ "${SERIAL}" = "${SERIAL#*BB}" ]; then
          fail "failed to extract serial number from i2c eeprom: " "${SERIAL}"
       fi
   else
       # NEW METHOD: matchs method '0' in aHw.C
